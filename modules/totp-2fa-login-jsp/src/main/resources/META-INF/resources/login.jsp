	...
	<c:otherwise>
		...
		<div class="login-container">
			...
			<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();" validateOnBlur="<%= false %>">
				...
				<aui:fieldset>
					...
					<liferay-util:dynamic-include key="com.liferay.login.web#/login.jsp#totp2fa" />
		...
		<aui:script sandbox="<%= true %>">
			...
			if (form) {
				...
				// The 2FA Authenticator Code field is injected into this form via
				// the totp2fa dynamic-include above. A Liferay Senna / SPA bug
				// unexpectedly prevents form body fields added this way being
				// included in the parameterMap passed through to the
				// Authenticator, so Senna is turned off for this form to ensure
				// it doesn't prevent the Authenticator functioning as expected.
